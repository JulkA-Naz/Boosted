(() => {
    ...
    (() => {
        ...
        const deleteProducts = productParent => {
            ...
            document.querySelector(`.item[data-id="${id}"]`).querySelector(".product__btn").disabled = false;
        ...
        const orderBtn = document.querySelector('[data-popup="#popup-ordering"]');
        const orderModal = document.querySelector("#popup-ordering");
        const orderQuantity = orderModal.querySelector(".order-modal__quantity span");
        const orderSum = orderModal.querySelector(".order-modal__summ span");
        const orderProductsList = orderModal.querySelector(".order-modal__list");
        orderBtn.addEventListener("click", (() => {
            orderQuantity.textContent = cartQuantity.textContent;
            orderSum.textContent = fullPrice.textContent;
            orderProductsList.innerHTML = "";
            const cartProducts = cartProductsList.querySelectorAll(".cart-content__item");
            cartProducts.forEach((product => {
                const productImg = product.querySelector(".cart-product__img").getAttribute("src");
                const productTitle = product.querySelector(".cart-product__title").textContent;
                const productPrice = product.querySelector(".cart-product__price").textContent;
                const orderProduct = generateOrderProduct(productImg, productTitle, productPrice);
                orderProductsList.insertAdjacentHTML("beforeend", orderProduct);
            }));
        }));
        const generateOrderProduct = (img, title, price) => `\n    <li class="order-modal__item">\n      <article class="order-modal__product order-product">\n        <img src="${img}" alt="" class="order-product__img">\n        <div class="order-product__text">\n          <h4 class="order-product__title">${title}</h4>\n          <span class="order-product__price">${price}</span>\n        </div>\n      </article>\n    </li>\n  `;
        window["FLS"] = true;
        isWebp();
        menuInit();
        tabs();
